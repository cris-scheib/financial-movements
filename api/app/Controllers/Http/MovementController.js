...
class MovementController {
    ...
    async create({ request, response, auth }) {
        try {
            const { name, value, movement_type_id } = request.all()
            const user = await auth.getUser()
            const movement = await Movement.create({ name: name, value: value, movement_type_id: movement_type_id, user_id: user.id })
    ...
    async update({ params, response, request }) {
        try {
            ...
            const data = request.only(['name', 'value', 'movement_type_id'])
            movement.merge(data)
            await movement.save()
            return response.status(200).json(movement);
        } catch (error) {
